...
namespace
{
json loadModelJson(const std::filesystem::path& modelDirectory, const char* filename)
{
    const auto path = modelDirectory / filename;
    std::ifstream stream(path);
    if (!stream.is_open()) {
        throw std::runtime_error(std::string("Basic Pitch model resource is missing: ") + filename);
    }
    return json::parse(stream);
}
} // namespace

BasicPitchCNN::BasicPitchCNN(const std::filesystem::path& modelDirectory)
{
    const auto json_cnn_contour = loadModelJson(modelDirectory, "cnn_contour_model.json");
    mCNNContour.parseJson(json_cnn_contour);

    const auto json_cnn_note = loadModelJson(modelDirectory, "cnn_note_model.json");
    mCNNNote.parseJson(json_cnn_note);

    const auto json_cnn_onset_input = loadModelJson(modelDirectory, "cnn_onset_1_model.json");
    mCNNOnsetInput.parseJson(json_cnn_onset_input);

    const auto json_cnn_onset_output = loadModelJson(modelDirectory, "cnn_onset_2_model.json");
